...
const PlayingVideoInfo = () => {
  ...
  const fetchData = async () => {
    const data = await fetch(
      `https://youtube.googleapis.com/youtube/v3/channels?part=snippet%2CcontentDetails%2Cstatistics&id=${info.snippet.channelId}&key=AIzaSyADJHNQq-nUrY0D0YeyLODt7OFkPPfoxW0`
    );
  ...
  return (
    <div className="md:w-full w-screen p-2">
      <div>
        <div>
          <h1 className="text-xl font-bold py-2">{info.snippet.title}</h1>
        </div>
        <div className="flex md:flex-row flex-col">
          <div className="flex md:w-4/12 justify-between mb-2">
            <div className="flex  items-center ">
              ...
              <div className="pr-5">
                <h1 className="font-medium">{info.snippet.channelTitle}</h1>
            ...
            <div className="flex items-center">
              <button className="bg-black text-white px-4 py-2 font-medium rounded-3xl">
                Subscribe
              </button>
            </div>
          </div>
          <div className="flex md:justify-end justify-end md:w-8/12 w-full overflow-x-scroll scrollbar-none">
            <div className="flex items-center overflow-x-scroll scrollbar-none">
              <div className="flex px-4 py-2 items-center border  mx-1 border-gray-300 rounded-3xl">
                ...
                {formatViewCount(info.statistics.likeCount)+"|"}
        ...
        <div className="bg-gray-100 rounded-xl mt-3 h-24  p-3">
          <div>
            <h1 className="font-medium">
              {formatViewCount(info.statistics.viewCount)} views{" "}
              <span className="mx-2"></span>
              {formatPublishedDate(info.snippet.publishedAt)}
            </h1>
          </div>
          <div>
